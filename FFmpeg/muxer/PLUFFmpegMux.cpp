...
//             rb->buf = (uint8_t*)realloc(rb->buf, new_cap);
//             rb->capacity = new_cap;
//         }
//
//         rb->size = size;
...
//             return false;
//
//         audio = (struct audio_params*)calloc(1, sizeof(*audio) * params->tracks);
//
...
//     context->time_base = {
//         ffm->params.fps_den, ffm->params.fps_num
//     };
//
...
//
//     stream->time_base = { 1, ffm->audio[idx].sample_rate };
//
...
//         ffm->audio_streams =
//             (AVStream**)calloc(1, ffm->params.tracks * sizeof(void*));
//
//         for (int i = 0; i < ffm->params.tracks; i++)
...
//     header->data = (uint8_t*)malloc(size);
...
//         "=", " ", 0))) {
//         printf("Failed to parse muxer settings: %d\n%s",
//             (ret), ffm->params.muxer_settings);
//
//         av_dict_free(&dict);
//     }
//
...
//         NULL, NULL);
//     if (ret < 0) {
//         printf("Couldn't initialize output context: %d\n",
//             (ret));
//         return FFM_ERROR;
//     }
//
...
//     if (ffm->params.tracks) {
//         ffm->audio_header =
//             (struct header*)calloc(1, sizeof(struct header) * ffm->params.tracks);
//     }
//
//     av_register_all();
//
...
//         stream->codec->time_base, stream->time_base, (enum AVRounding)(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
// }
//
...
static AVStream* plu_new_stream(PLUMuxContext *mux_ctx, AVFormatContext  *out_fmt_ctx, const char *codec_name) {
    CodecParam *codec_info = &mux_ctx->codec_info;
...
static int plu_init_output(PLUMuxContext *mux_ctx)
{
    ...
    output_format = av_guess_format(NULL, codec_info->file_name, NULL);
    if (output_format == NULL) {
        LOG_ERROR("Couldn't find an appropriate muxer for '%s'\n", codec_info->file_name);
        return -1;
    }
    ...
    if (codec_info->has_video) {
        ...
        AVStream *stream;
        if ((stream = plu_new_stream(mux_ctx, out_fmt_ctx, codec_info->v_codec_name)) == NULL)
            goto ERROR_EXIT;

        codec_ctx = stream->codec;
        codec_ctx->bit_rate = codec_info->v_bitrate * 1000;
        codec_ctx->width = codec_info->width;
        codec_ctx->height = codec_info->height;
        codec_ctx->coded_width = codec_info->width;
        codec_ctx->coded_height = codec_info->height;
        codec_ctx->extradata_size = codec_info->v_extradata_size;
        codec_ctx->extradata = (uint8_t*)av_memdup(codec_info->v_extradata, codec_ctx->extradata_size);
        codec_ctx->time_base = { codec_info->fps_den, codec_info->fps_num };
        codec_ctx->framerate = { codec_info->fps_num , codec_info->fps_den };
        codec_ctx->codec_tag = 0;
        if (out_fmt_ctx->oformat->flags & AVFMT_GLOBALHEADER)
            codec_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
        mux_ctx->video_index = out_fmt_ctx->nb_streams - 1;
    }
    if (codec_info->has_audio) {
        AVCodecContext *codec_ctx = NULL;
        AVStream *stream;
        if ((stream = plu_new_stream(mux_ctx, out_fmt_ctx, codec_info->a_codec_name)) == NULL)
            goto ERROR_EXIT;

        codec_ctx = stream->codec;
        codec_ctx->bit_rate = codec_info->a_bitrate * 1000;
        codec_ctx->channels = codec_info->audio_channel;
        codec_ctx->sample_rate = codec_info->samplerate;
        codec_ctx->sample_fmt = AV_SAMPLE_FMT_S16;
        codec_ctx->extradata_size = codec_info->a_extradata_size;
        codec_ctx->extradata = (uint8_t*)av_memdup(codec_info->a_extradata, codec_ctx->extradata_size);
        codec_ctx->channel_layout = av_get_default_channel_layout(codec_ctx->channels);
        codec_ctx->time_base = { 1, codec_ctx->sample_rate };
        codec_ctx->codec_tag = 0;
        if (out_fmt_ctx->oformat->flags & AVFMT_GLOBALHEADER)
            codec_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
        mux_ctx->audio_index = out_fmt_ctx->nb_streams - 1;
    }
    av_dump_format(out_fmt_ctx, 0, codec_info->file_name, 1);
...
PLUHandler plumux_initparam(const CodecParam *param)
{
    ...
    memcpy(&mux_ctx->codec_info, param, sizeof(*param));
...
int plumux_write_packet(PLUHandler pHandler, const plu_packet_info *packet_info)
{
    ...
    if (packet_info->type == PACKET_VIDEO)
        index = mux_ctx->video_index;
    else if (packet_info->type == PACKET_AUDIO)
        index = mux_ctx->audio_index;
